...
from src.packages.safe_rlhf.trainers import RLTrainer
...
class PPOLagTrainer(RLTrainer):
    ...
    def __init__(
        ...
        super().__init__(
            args=args, ds_train_config=ds_train_config, ds_eval_config=ds_eval_config
        )
        ...
        self.log_lambda_max = (
            np.log(self.args.lambda_max) if self.args.lambda_max else None
        )
        self.log_lambda_optimizer = torch.optim.SGD(
            [self.log_lambda], lr=self.args.lambda_lr
        )
        ...
        if not is_same_tokenizer(self.tokenizer, self.cost_critic_tokenizer):
            raise ValueError(
                (
                    ...
                    " Expected {0.__module__}.{0.__qualname__}(vocab_size={1}), but got"
                    " {2.__module__}.{2.__qualname__}(vocab_size={3}). Please consider"
                    " pass `--cost_critic_model_name_or_path` from the command line."
                ).format(
                    type(self.tokenizer),
                    len(self.tokenizer),
    ...
    def rl_step(self, rl_batch: dict[str, torch.Tensor]) -> dict[str, Any]:
        ...
        with torch.no_grad():
            ...
            kl_divergence = (
                ((old_log_probs - ref_log_probs)[:, start:] * mask).sum(dim=-1).mean()
            )
        ...
        return {
            ...
            "train/reward_critic_lr": self.reward_critic_model.optimizer.param_groups[
                0
            ]["lr"],
            "train/cost_critic_lr": self.cost_critic_model.optimizer.param_groups[0][
                "lr"
            ],
