...
from src.packages.safe_rlhf.datasets import PreferenceDataset
from src.packages.safe_rlhf.models import load_pretrained_models
from src.packages.safe_rlhf.trainers import SupervisedTrainer
from src.packages.safe_rlhf.utils import gather_log_probabilities, get_all_reduce_mean
...
class DPOTrainer(SupervisedTrainer):
    ...
    def loss(  # pylint: disable=too-many-locals
        ...
        for i in range(batch_size):
            ...
            assert (
                0 <= diverge_index <= better_end_index
            ), "diverge index is out of range!"
            assert (
                0 <= diverge_index <= worse_end_index
            ), "diverge index is out of range!"
            ...
            ref_better_log_prob = ref_better_sequence_log_probs[
                i, better_seq_slice
            ].sum(dim=-1)
            ref_worse_log_prob = ref_worse_sequence_log_probs[i, worse_seq_slice].sum(
                dim=-1
            )
            ...
            losses.append(
                -F.logsigmoid(self.scale_coeff * (better_log_ratio - worse_log_ratio))
            )
        ...
        reward_accuracy = (
            (better_sample_reward > worse_sample_reward).float().mean()
        )  # size = ()
