...
from src.packages.safe_rlhf.configs import ADAM_BETAS
from src.packages.safe_rlhf.datasets import (
    DummyDataset,
    PromptOnlyBatch,
    PromptOnlyDataset,
    SupervisedDataset,
...
from src.packages.safe_rlhf.trainers.base import TrainerBase
...
class RLTrainer(TrainerBase):  # pylint: disable=too-many-instance-attributes
        ...
        if not is_same_tokenizer(self.tokenizer, self.reward_critic_tokenizer):
            raise ValueError(
                (
                    ...
                    " pass `--reward_critic_model_name_or_path` from the command line."
                ).format(
                    type(self.tokenizer),
                    len(self.tokenizer),
    ...
    def init_datasets(self) -> None:
        ...
        if (
            ...
            * self.args.num_return_sequences
            ...
            raise ValueError(
                "The number of prompt-only samples must be divisible by the micro batch"
                " size.",
            )
        ...
        if self.args.need_eval:
            if (
                ...
                prompt_only_dataset, eval_dataset = (
    ...
    def _init_train_engine(
        ...
        lr_scheduler_update_steps = (
            total_training_steps // ds_config["gradient_accumulation_steps"]
        )
