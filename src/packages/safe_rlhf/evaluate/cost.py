...
from src.packages.safe_rlhf.datasets import SafetyPreferenceDataset, parse_dataset
...
def main() -> None:  # pylint: disable=too-many-locals,too-many-statements
    ...
    for batch in dataloader:
        ...
        with torch.no_grad():
            ...
            num_correct_sign_predictions += (
                lower_end_costs * batch["safer_safety_sign"] < 0
            ).sum()
            num_correct_sign_predictions += (
    ...
    if dist.is_initialized() and dist.get_world_size() > 1:
        ...
        storage_scores_cated = torch.cat(
            [storage_safe_scores, storage_unsafe_scores], dim=0
        )
        storage_scores_lst = [
    ...
    if is_main_process():
        print(
            f"accuracy: {accuracy:.4f}, accuracy_sign: {accuracy_sign:.4f}, scores:"
            f" {scores:.4f}"
        )
        ...
        hist, bin_edges = np.histogram(
            scores, bins=100, range=(min(scores), max(scores))
        )
        ...
        hist, bin_edges = np.histogram(
            safe_scores, bins=100, range=(min(scores), max(scores))
        )
        plt.bar(bin_edges[:-1], hist, width=bin_edges[1] - bin_edges[0], align="edge")
        plt.xlabel("Score range")
        plt.ylabel("Frequency")
        ...
        hist, bin_edges = np.histogram(
            unsafe_scores, bins=100, range=(min(scores), max(scores))
        )
        plt.bar(bin_edges[:-1], hist, width=bin_edges[1] - bin_edges[0], align="edge")
        plt.xlabel("Score range")
        plt.ylabel("Frequency")
