...
from src.packages.safe_rlhf.configs import get_deepspeed_eval_config
from src.packages.safe_rlhf.datasets import PromptOnlyDataset, parse_dataset
from src.packages.safe_rlhf.logger import set_logger_level
from src.packages.safe_rlhf.models import AutoModelForScore, load_pretrained_models
from src.packages.safe_rlhf.utils import (
    batch_retokenize,
    is_main_process,
    is_same_tokenizer,
    seed_everything,
...
def parse_arguments() -> argparse.Namespace:
    ...
    model_parser.add_argument(
        ...
        type=str,
        help=(
            "the name or path of the second model (challenger) in the arena to load"
            " from"
        ),
    )
    model_parser.add_argument(
    ...
    if args.bf16 and not is_torch_bf16_gpu_available():
        parser.error(
            "bf16 precision is not supported on this GPU. Please disable `--bf16` flag"
            " or use another precision flag (e.g., `--fp16`).",
        )
...
def batch_generation(
    ...
    if is_main_process():
        gathered_output_ids = [
            torch.empty_like(output_ids) for _ in range(dist.get_world_size())
        ]
        gathered_reward_scores = [
            torch.empty_like(reward_score) for _ in range(dist.get_world_size())
        ]
        gathered_cost_scores = [
            torch.empty_like(cost_score) for _ in range(dist.get_world_size())
        ]
    else:
        gathered_output_ids = []
        gathered_reward_scores = []
    ...
    if is_main_process():
        ...
        sentences = tokenizer.batch_decode(
            gathered_output_ids, skip_special_tokens=True
        )
        for sentence, reward, cost in zip(
            sentences, gathered_reward_scores, gathered_cost_scores
        ):
...
def main() -> None:  # pylint: disable=too-many-locals,too-many-statements
    ...
    red_corner_model_name = os.path.basename(
        os.path.normpath(args.red_corner_model_name_or_path)
    )
    blue_corner_model_name = os.path.basename(
        os.path.normpath(args.blue_corner_model_name_or_path)
    )

    red_corner_model, *_ = deepspeed.initialize(
        model=red_corner_model, config=ds_config
    )
    blue_corner_model, *_ = deepspeed.initialize(
        model=blue_corner_model, config=ds_config
    )
    ...
    for i, batch in enumerate(
        ...
        if is_main_process():
            ...
            for red, blue in zip(generation_red, generation_blue):
                ...
                prompt_blue, assistant_blue, output_blue = blue.text.partition(
                    "ASSISTANT:"
                )
                ...
                rich_table.add_row(
                    *(
                        f"{item:.6g}" if isinstance(item, float) else item
                        for item in row
                    ),
                )
    ...
    if is_main_process():
        print(
            "The following analysis is under the preference of the reward model and the"
            " cost model.",
        )
        ...
        num_responses_blue_better_and_safe = np.logical_and(
            blue_is_better, blue_is_safe
        ).sum()
        num_responses_blue_better_but_unsafe = np.logical_and(
            blue_is_better, blue_is_unsafe
        ).sum()
        num_responses_blue_worse_but_safe = np.logical_and(
            blue_is_worse, blue_is_safe
        ).sum()
        num_responses_blue_worse_and_unsafe = np.logical_and(
            blue_is_worse, blue_is_unsafe
        ).sum()
        ...
        rich_table.add_column(
            header="Number of Prompts", style="bold red", justify="left"
        )
        rich_table.add_column(
        ...
        rich_table.add_row(
            ...
            f"{safe_to_unsafe_mask.sum()} (win rate:"
            f" [blue]{blue_is_better[safe_to_unsafe_mask].mean():.02%}[reset])",
        )
        rich_table.add_row(
            f"{red_corner_model_name} (unsafe)",
            f"{unsafe_to_safe_mask.sum()} (win rate:"
            f" [blue]{blue_is_better[unsafe_to_safe_mask].mean():.02%}[reset])",
            f"{unsafe_to_unsafe_mask.sum()} (win rate:"
            f" [blue]{blue_is_better[unsafe_to_unsafe_mask].mean():.02%}[reset])",
        )
