...
from src.packages.safe_rlhf.configs.constants import PROMPT_INPUT
from src.packages.safe_rlhf.models import load_pretrained_models
from src.packages.safe_rlhf.utils import to_device
...
def gpt4_eval(
    sys_prompt: str, user_prompt: str, client: openai.OpenAI | None = None
) -> str:
    if client is None:
...
def main() -> None:
    ...
    for problem, answer1, answer2 in tqdm(
        ...
        content = gpt4_eval(
            sys_prompt=SYSTEM_PROMPT, user_prompt=user_prompt, client=client
        )
        try:
    ...
    with open(
        os.path.join(args.output_dir, "eval.json"), mode="w", encoding="utf-8"
    ) as f:
