...
from src.packages.safe_rlhf.serve.chatbot import (
    CODE_BLOCK_PATTERN,
    ChatbotList,
    EndOfDialogue,
    ModelArgs,
...
from src.packages.safe_rlhf.utils import str2bool
...
class CLI:
    ...
    def __init__(self, *model_args: ModelArgs, stream: bool = True) -> None:
        ...
        self.console = Console(
            soft_wrap=False, markup=False, emoji=False, highlight=False
        )
        ...
        self.styles = [
            style for style, _ in reversed(tuple(zip(self.STYLES, self.chatbots)))
        ]
    ...
    def render(
        self, response_generator: Generator[str, None, None], name: str, style: str
    ) -> None:
        response = ""
        if self.stream:
            with Live(
                console=self.console, refresh_per_second=25, transient=True
            ) as live:
                prefix = "Generating...\n"
        ...
        self.console.print(
            f"[{self.chatbots.round}] Assistant (", style=f"bold {style}", end=""
        )
        ...
        while match := CODE_BLOCK_PATTERN.search(response):
            ...
            self.console.print(
                match.group("prefix"), style=f"bold italic {style}", end=""
            )
            ...
            self.console.print(
                match.group("suffix"), style=f"bold italic {style}", end=""
            )
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Talking to one or more model(s) in cli mode."
    )
    parser.add_argument(
        "--model_name_or_path",
        type=str,
