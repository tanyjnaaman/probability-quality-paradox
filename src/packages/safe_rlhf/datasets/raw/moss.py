...
class MOSS002SFT(RawDataset):
    ...
    def __init__(self, path: str | None = None) -> None:
        if path is None:  # fnlp/moss-002-sft-data cannot load with `load_dataset`
            raise ValueError(
                "moss-002-sft dataset requires a local path to the dataset."
            )
...
class MOSS003SFT(RawDataset):
    ...
    def __init__(self, path: str | None = None) -> None:
        if path is None:  # fnlp/moss-003-sft-data cannot load with `load_dataset`
            raise ValueError(
                "moss-003-sft dataset requires a local path to the dataset."
            )
    ...
    def __getitem__(self, index: int) -> RawSample:
        ...
        for i in range(1, num_turns + 1):
            turn = chat[f"turn_{i}"]
            dialogue.append(
                turn["Human"].replace("<|Human|>:", "").replace("<eoh>", "").strip()
            )
            dialogue.append(
                turn["MOSS"].replace("<|MOSS|>", "").replace("<eom>", "").strip()
            )
        return RawSample(dialogue=dialogue)
