...
from src.packages.safe_rlhf.utils import is_main_process
...
class RawSample(TypedDict, total=False):
    ...
    input: NotRequired[
        str | list[str]
    ]  # either `input` or `dialogue` should be provided
    """User input text."""
...
class RawDataset(Dataset[RawSample]):
    ...
    def load(name: str, /, *args: Any, **kwargs: Any) -> RawDataset:
        ...
        try:
            ...
            raise ValueError(
                f"Unknown dataset name `{name}`. You should implement a raw dataset"
                " with that name and import it in your code. Available datasets are:"
                f" {', '.join(sorted(RawDataset.__REGISTRY.keys()))}.",
            ) from ex
