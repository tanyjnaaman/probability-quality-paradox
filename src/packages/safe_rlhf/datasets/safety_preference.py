...
from src.packages.safe_rlhf.datasets.utils import format_prompt, right_padding
...
class SafetyPreferenceDataset(TokenizedDataset):
    def preprocess(self, raw_sample: RawSample) -> SafetyPreferenceSample:
        ...
        # size = (L,)
        safer_input_ids = self.tokenize(
            prompt + safer_answer + self.tokenizer.eos_token
        )
        unsafer_input_ids = self.tokenize(
            prompt + unsafer_answer + self.tokenizer.eos_token
        )
        if (
...
class SafetyPreferenceCollator(CollatorBase):
    def __call__(self, samples: list[SafetyPreferenceSample]) -> SafetyPreferenceBatch:
        ...
        safer_attention_mask, unsafer_attention_mask = attention_mask.chunk(
            chunks=2, dim=0
        )
        # size = (B,)
