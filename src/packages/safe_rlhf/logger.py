...
from src.packages.safe_rlhf.utils import is_main_process, rank_zero_only
        ...
        if cls._instance is None:
            ...
            if is_main_process():
                ...
                if log_dir is not None:
                    ...
                    with (log_dir / "arguments.json").open(
    ...
    def print_table(
        ...
        if data is not None:
            if columns is not None or rows is not None:
                raise ValueError(
                    "`logger.print_table` should be called with both `columns` and"
                    " `rows`, or call with `data`.",
                )
