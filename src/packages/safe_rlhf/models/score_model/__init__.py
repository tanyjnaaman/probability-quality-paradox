...
from src.packages.safe_rlhf.models.normalizer import NormalizeFunction, Normalizer
    ...
    def _load_attr_from_module(self, model_type: str, attr: str) -> Any:
        ...
        if module_name not in self._modules:
            self._modules[module_name] = importlib.import_module(
                f".{module_name}",
                "src.packages.safe_rlhf.models.score_model",
            )
...
class ScoreModelMixin:
    ...
    def init_score_head(
        self, config: PretrainedConfig, hidden_size: int, **kwargs: Any
    ) -> None:
        ...
        self.score_head = nn.Linear(
            hidden_size, config.score_dim, bias=config.score_bias
        )
        if config.score_bias:
            nn.init.zeros_(self.score_head.bias)

        config.score_type = kwargs.pop(
            "score_type", getattr(config, "score_type", "reward")
        )
        if config.score_type == "reward":
            ...
            raise ValueError(
                f"Invalid score type: {config.score_type}. Expected one of 'reward',"
                " 'cost', or 'critic'.",
            )
        ...
        if config.normalizer_type not in {
            ...
            raise ValueError(
                ...
                "Expected one of 'RunningMeanStd', 'ExponentialMovingAverage', or"
                " None.",
            )
