...
from src.packages.safe_rlhf.models.score_model import ScoreModelMixin, ScoreModelOutput
...
class GPT2ForScore(ScoreModelMixin, GPT2PreTrainedModel):
    ...
    def parallelize(self, device_map: str | None = None) -> None:
        warnings.warn(
            "`GPT2LMHeadModel.parallelize` is deprecated and will be removed in v5 of"
            " Transformers, you should load your model with `device_map='balanced'` in"
            " the call to `from_pretrained`. You can also provide your own `device_map`"
            " but it needs to be a dictionary module_name to device, so for instance"
    ...
    def deparallelize(self) -> None:
        warnings.warn(
            "Like `parallelize`, `deparallelize` is deprecated and will be removed in"
            " v5 of Transformers.",
            FutureWarning,
            stacklevel=1,
        )
    ...
    def forward(  # pylint: disable=too-many-arguments
        ...
        ```python
        >>> from src.packages.safe_rlhf.models.score_model.llama.modeling_llama import LlamaForScore
