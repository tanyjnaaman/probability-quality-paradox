...
from src.packages.safe_rlhf.datasets import SafetyPreferenceDataset
from src.packages.safe_rlhf.models import AutoModelForScore, ScoreModelOutput
from src.packages.safe_rlhf.trainers import SupervisedTrainer
...
class CostTrainer(SupervisedTrainer):
    ...
    def eval(self) -> dict[str, Any]:  # pylint: disable=too-many-locals
        ...
        if is_main_process():
            gathered_costs = [
                torch.empty_like(costs) for _ in range(dist.get_world_size())
            ]
        else:
            gathered_costs = []
        ...
        if is_main_process():
            ...
            l_is_safe = [
                str(l_sign >= 0) for l_sign in batch["safer_safety_sign"].tolist()
            ]
            l_costs = [f"{cost:.6f}" for cost in lower_end_costs.tolist()]
            h_is_safe = [
                str(h_sign >= 0) for h_sign in batch["unsafer_safety_sign"].tolist()
            ]
    ...
    def loss(
        ...
        """
        assert safer_input_ids.size(0) == unsafer_input_ids.size(
            0
        ), "batch size mismatch!"
        ...
        output: ScoreModelOutput = self.model(
            ...
            attention_mask=torch.cat(
                [safer_attention_mask, unsafer_attention_mask], dim=0
            ),
        )
        ...
        # size = (B,)
        lower_end_cost, higher_end_cost = end_scores.squeeze(dim=-1).chunk(
            chunks=2, dim=0
        )
        ...
        if self.args.loss_type == "token-wise":
            ...
            for i in range(batch_size):
                ...
                higher_end_index = (
                    unsafer_attention_mask[i].nonzero()[-1].squeeze().item()
                )
                end_index = max(higher_end_index, lower_end_index)

                diverge_index = (
                    (safer_input_ids[i] != unsafer_input_ids[i])
                    .nonzero()[0]
                    .squeeze()
                    .item()
                )
